...
void bench_add_cleanup(void *user) {
...
int main() {
	...
	bench("intern lookup", NULL, bench_lookup_run, NULL, &i);

	intern_free(&i);
	bench("intern add (no preallocate)", bench_add_setup, bench_add_run, bench_add_cleanup, &i);
	bench("intern add (preallocate)", bench_add_setup_prealloc, bench_add_run, bench_add_cleanup, &i);
}
